...
export const syncService = {
  ...
  generatePHPScript(dbHost: string, dbUser: string, dbPass: string, dbName: string, scoutToken: string, seniorToken: string): string {
    return `<?php
/**
 * BRIDGE SCOUT RPG - Multi-app via Token
 * Suporta: scout276 (patrols/logs) e senior276 (patrols_senior/logs_senior)
 */
...
// Mapa: token => [tabela_patrulhas, tabela_logs, patrulhas_iniciais]
\$tableMap = [
    '${scoutToken}' => [
        'patrols' => 'patrols',
        'logs'    => 'logs',
    ...
    '${seniorToken}' => [
        'patrols' => 'patrols_senior',
        'logs'    => 'logs_senior',
        'initial' => [
            ['s1', 'Kaaete',  'Cataratas do Iguaçu'],
            ['s2', 'Kuruqui', 'Cataratas do Iguaçu'],
        ],
    ],
];
...
if (empty(\$receivedToken) || !isset(\$tableMap[\$receivedToken])) {
    http_response_code(403);
    echo json_encode(["error" => "AUTH_FAILED"]);
    exit;
}

\$config        = \$tableMap[\$receivedToken];
\$patrolsTable  = \$config['patrols'];
\$logsTable     = \$config['logs'];
\$initialPatrols = \$config['initial'];
...
\$conn->query("
CREATE TABLE IF NOT EXISTS \`{\$patrolsTable}\` (
...
\$result = \$conn->query("SELECT COUNT(*) as count FROM \`{\$patrolsTable}\`");
\$row = \$result->fetch_assoc();
if (\$row['count'] == 0) {
    \$stmt = \$conn->prepare("INSERT INTO \`{\$patrolsTable}\` (id, name, troop) VALUES (?, ?, ?)");
    foreach (\$initialPatrols as \$p) {
    ...
    if (\$data && isset(\$data['action']) && \$data['action'] === 'delete_logs') {
        ...
        try {
            ...
            if (count(\$logIds) > 0) {
                ...
                \$res = \$conn->query("SELECT * FROM \`{\$logsTable}\` WHERE id IN (\$idsStr)");
                while (\$row = \$res->fetch_assoc()) { \$logsToDelete[] = \$row; }
            }

            if (count(\$groupIds) > 0) {
                \$groupsStr = "'" . implode("','", array_map([\$conn, 'real_escape_string'], \$groupIds)) . "'";
                \$res = \$conn->query("SELECT * FROM \`{\$logsTable}\` WHERE group_id IN (\$groupsStr)");
                while (\$row = \$res->fetch_assoc()) {
                    \$exists = false;
                    foreach (\$logsToDelete as \$l) {
                        if (\$l['id'] === \$row['id']) { \$exists = true; break; }
                    }
                    if (!\$exists) \$logsToDelete[] = \$row;
                }
            }
            ...
            foreach (\$logsToDelete as \$log) {
                ...
                if (\$col) {
                    \$stmtUpdate = \$conn->prepare("UPDATE \`{\$patrolsTable}\` SET \$col = GREATEST(0, \$col - ?) WHERE name = ?");
                    \$stmtUpdate->bind_param("is", \$log['xp_value'], \$log['patrol_name']);
                    \$stmtUpdate->execute();
                }
                \$stmtDel = \$conn->prepare("DELETE FROM \`{\$logsTable}\` WHERE id = ?");
        ...
        \$conn->begin_transaction();
        try {
            \$stmtLog = \$conn->prepare("INSERT IGNORE INTO \`{\$logsTable}\` (id, timestamp, patrol_name, xp_value, attribute, description, comment, group_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
\$patrols = [];
\$resPatrols = \$conn->query("SELECT * FROM \`{\$patrolsTable}\`");
...
\$logs = [];
\$resLogs = \$conn->query("SELECT * FROM \`{\$logsTable}\` ORDER BY timestamp DESC LIMIT 100");
